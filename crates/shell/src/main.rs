...
pub struct View {
    settings: abi::ViewSettings,

    root: ViewNode,
    elements: slotmap::SlotMap<ViewNode, abi::ElementInfo>,
    children: slotmap::SecondaryMap<ViewNode, Vec<ViewNode>>,
    parents: slotmap::SecondaryMap<ViewNode, Option<ViewNode>>,

    events: VecDeque<abi::ViewEvent>,

    mouse_pos: abi::Xy<f32>,
...
impl View {
    ...
    pub fn handle_mouse_move(&mut self, position: abi::Xy<f32>) {
        ...
        // queue?
        self.events.push_back(abi::ViewEvent::MouseMove { delta });
        ...
        if self.mouse_over != new_mouse_over {
            for node in &self.mouse_over {
                if !new_mouse_over.contains(node) {
                    self.events.push_back(abi::ViewEvent::MouseLeave {
                        node: node.0.as_ffi(),
                    });
                }
            }
            for node in &new_mouse_over {
                if !self.mouse_over.contains(node) {
                    self.events.push_back(abi::ViewEvent::MouseEnter {
                        node: node.0.as_ffi(),
                    });
                }
            }
    ...
    pub fn handle_mouse_down(&mut self, button: abi::MouseButton, now: Instant) {
        ...
        {
            match button {
                abi::MouseButton::Primary => {
                    self.events.push_back(abi::ViewEvent::Click {
                        node: node.0.as_ffi(),
                    });
                    if self
                    ...
                    {
                        if now.duration_since(self.last_left_click_time).as_secs_f64()
                            < self.settings.double_click_timeout
                        {
                            self.events.push_back(abi::ViewEvent::DoubleClick {
                                node: node.0.as_ffi(),
                            });
                        }
                    } else {
                ...
                abi::MouseButton::Secondary => self.events.push_back(abi::ViewEvent::RightClick {
                    node: node.0.as_ffi(),
                }),
                _ => {}
            }
            ...
            .find(|node| self.elements[**node].focusable())
        {
            ...
            match button {
                abi::MouseButton::Primary | abi::MouseButton::Secondary => {
                    let old = self.focus.take();
                    self.focus = Some(*node);
                    self.events.push_back(abi::ViewEvent::Focus {
                        old: old.map(|n| n.0.as_ffi()),
                        new: node.0.as_ffi(),
                    });
                }
                _ => {}
